...
import (
	...
	"choopong.com/n-rabbit-consumers/cmd/manager/processor"
...
var workerExchange string
var numWorkers int

func init() {
	rabbitMQURI = os.Getenv("RABBIT_MQ_URI")
	rabbitMQName = os.Getenv("RABBIT_MQ_MANAGER_QUEUE_NAME")
	rabbitMQRetryExchange = os.Getenv("RABBIT_MQ_MANAGER_RETRY_EXCHANGE")
	workerExchange = os.Getenv("RABBIT_MQ_WORKER_EXCHANGE")
...
func main() {
	...
	cProcessor := processor.NewProcessor(qConn.GetChannel(), workerExchange, numWorkers)
